...
- only for aftershocks with larger haversine distance from theirs parent
...
dPar = {'a_Mc': np.array([2.5]),  # np.array([3.0, 4.0]),
...
for i in range(dPar['a_Mc'].shape[0]):
    ...
    a_T1 = a_T[sel_TR]
    a_R1 = a_R[sel_TR]
    # select only clustered events
    sel_cluster = np.log10(a_T1*a_R1) <= -5
    ax.scatter(np.log10(a_T1[sel_cluster]),np.log10(a_R1[sel_cluster]),s=.5,c='black',alpha=0.5)
    # =================================5==============================================
    #                           save results
    # ================================================================================
    print('plot saved in: ', 'plots/g_RL_clustered_%.1f.png' % (dPar['R_L']))
    plt.figure(1)
    plt.savefig('plots/g_RL_clustered_%.1f.png' % (dPar['R_L']))
    plt.show()
    plt.clf()