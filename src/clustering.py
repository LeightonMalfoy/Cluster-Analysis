...
def NND_eta( eqCat, dConst, **kwargs):
    ...
    - {  'aNND'       : aNND,     - nearest neighbor space-time magnitude distance
         ...
         'aHD'        : np.array  - haversine distance between earthquake and its parent
        } 
    ...
    aHD      = np.zeros(  eqCat.size()) # havensine distance between earthquake and its parent
    ...
    for jC in range( eqCat.size()):
        ...
        if sel_tau_par.sum() > 0:
            ...
            #  haversine distance
            vHD = haversine( eqCat.data['Lon'][jC], eqCat.data['Lat'][jC],eqCat.data['Lon'][vcurr_ID], eqCat.data['Lat'][vcurr_ID] )
            sel_r_par = vR < rmax
            if sel_r_par.sum() > 0:
                ...
                aHD[jC]     = vHD[sel_min][0] # haversine unit:meter, convert meter to km
    ...
    return {  'aNND' : aNND[aNND>0], 'aEqID_p' : vID_p[aNND>0], 'aEqID_c' : vID_c[aNND>0], 'aHD':aHD[aNND>0]}
