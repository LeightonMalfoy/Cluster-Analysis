...
import src.date_time_utils as dateTime
...
class EqCat:
    ...
    def merge(self,catalog1,catalog2):
        import copy
        try:
            for tag, vector in catalog1.data.items():
                self.data[tag] = copy.copy(catalog1.data[tag])
        except:
            for tag, vector in catalog1.items():
                self.data[tag] = copy.copy(catalog1[tag])
        try:
            for tag, vector in catalog2.data.items():
                self.data[tag]=np.concatenate((self.data[tag], copy.copy(catalog2.data[tag])),axis=0)
        except:
            for tag, vector in catalog2.items():
                self.data[tag] =np.concatenate((self.data[tag], copy.copy(catalog2[tag])),axis=0)
    ...
    def loadEqCat(self, file_in, catalogType, **kwargs):
        ...
        """
        import src.date_time_utils as dt_utils
        # ----------------check kwargs---------------------------------
        ...
        if catalogType == 'HS_reloc':
            ...
            headList = ['YR', 'MO', 'DY', 'HR', 'MN', 'SC', 'N', 'Lat', 'Lon', 'Depth', 'Mag', 'nPick', 'distSta',
                        'rms', 'd/n', 'rMeth', 'clID', 'nEvInCl', 'nlnk', 'err_h', 'err_z', 'rel_err_H', 'rel_err_Z']
            self.data = {}
            mData = np.loadtxt(file_in, skiprows=header)
            # mData = mData.T
            print('no of columns', mData[0].shape[0])
            print('no. of earthquakes', mData[:, 0].shape[0])
            for l in list(range(mData[0].shape[0])):
            ...
            vTime = np.array([])
            for i in list(range(mData[0].shape[0])):
                vTime = np.append(vTime, dateTime.datetime2decYr(np.array([mData[0][i], mData[1][i], mData[2][i], mData[3][i], mData[4][i], mData[5][i]])))
            self.data['Time'] = vTime

            for i in list(range(len(vStrHeader) - 1)):
        ...
        for i in list(range(self.data['Mag'].shape[0])):
            print(i + 1, 'out of', self.data['Mag'].shape[0])
            YR, MO, DY, HR, MN, SC = dt_utils.checkDateTime(
                [self.data['YR'][i], self.data['MO'][i], self.data['DY'][i], self.data['HR'][i], self.data['MN'][i],
                 self.data['SC'][i]])
            self.data['Time'] = np.append(self.data['Time'],dateTime.datetime2decYr(np.array([YR, MO, DY, HR, MN, SC])))
        self.data.pop('YR')
        self.data.pop('MO')
        self.data.pop('DY')
        self.data.pop('HR')
    ...
    def selectEvents(self, min, max, tag, **kwargs):
        ...
        example: selector( 3, 5, 'Mag', includeBoundaryEvents = True) - all events between 3 and 5 including M=3 and M=5 events
                 selector( 3, None, 'Mag')  - everything above M=3 excluding M=3 events
                 selector( 4, None, 'Mag') and then selector( 'w', None, 'MagType') - all Mws above Mw = 4
