...
dPar  = {   'a_Mc'        :  np.array( [2.5, 3.0, 3.5, 4.0]), #np.array( [2.0, 2.5, 3.0, 3.5]),np.array([3.0, 4.0])
...
for f_Mc in dPar['a_Mc']:
    ...
    NND_file = 'df1.7/%s_NND_Mc_%.1f.mat'%(os.path.basename( file_in).split('.')[0], f_Mc)
    ...
    #================================================================================
    plt.figure(1)
    plt.savefig( '%s/df1.7/%s_spanningTree_Mc_%.1f.png'%(plot_dir, file_in.split('.')[0], f_Mc))
